...
// @namespace
// @version           1.0.2
// @description       自用脚本, 自动跳过视频广告, 当前支持 腾讯, 爱奇艺, 优酷, 搜狐, 芒果tv
// @author            daniel
...
(function () {
    ...
    switch (host) {
        ...
        case 'v.qq.com':  // 腾讯视频
            setInterval(() => {   // 视频广告加速
                var $txpAd = $(".txp_ad")
                var guanggao1 = $txpAd.find("txpdiv").find("video")[0] // find()[0] dom对象
                var guanggao2 = $txpAd.find("txpdiv").find("video")[1]
                if (guanggao1 && guanggao1.currentTime) {
                    guanggao1.currentTime = 1000 // 将广告跳到第 1 秒
                }
                if (guanggao2 && guanggao2.currentTime) {
                    guanggao2.currentTime = 1000
                }
            }, 1000)
            break

        case 'v.youku.com':  // 优酷视频
            // 等到广告出现,设置display为 none,然后点击一次播放, 清除interval
            var interval_youku = setInterval(() => {
                // 先关闭广告声音
                let $sound = $('#ykPlayer > div:eq(7) > div > div:eq(2) > span:eq(1)')
                if ($sound) {
                    let classes = $sound.attr('class')
                    let length = classes.split(' ').length
                    if (length === 4) {
                        // 处于非静音状态(class长度为5处于静音状态)
                        // 点击静音
                        $sound.click()
                    }
                }
                // 隐藏广告窗口
                var guanggao_div = $(".h5-ext-layer > div").get(0);
                // console.log(guanggao_div)
                if (guanggao_div) {
                    guanggao_div.style.display = 'none';

                    // 点击播放按钮
                    var bofang = document.getElementsByClassName("control-play-icon")[0]
                    if (bofang) {
                        bofang.click()
                        clearInterval(interval_youku)
                    }
                }
            }, 1000)
            break

        case 'www.mgtv.com':   // 芒果tv(待完成)
            setInterval(() => {
                let $ad = $('body').find('video')
                if ($ad) {
                    let srcVal = $ad.attr('src');
                    console.log(srcVal)
                    if (srcVal && !srcVal.startsWith('blob:')) {
                        $ad.get(0).currentTime = 1000
                    }
                }
            }, 1000)
            break;

        case 'tv.sohu.com':  // 搜狐视频(待完成)
            setInterval(() => {
                let ad = $('#player > shpdiv > shpdiv.x-video-adv > video').get(0)
                if (ad && ad.currentTime) {
                    ad.currentTime = 1000
                }
            }, 1000)
            break
